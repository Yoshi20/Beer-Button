...
task :remote_environment do
  ...
  invoke :'rvm:use', 'ruby-3.3.0@beerbutton'
end

# Put any custom commands you need to run at setup
# All paths in `shared_dirs` and `shared_paths` will be created on their own.
task :setup do
  #command %{rvm install ruby-3.3.0}
...
task :stop_delayed_job_worker do
  ...
  command %{RAILS_ENV=production /var/www/beerbutton/current/bin/delayed_job stop}
end

task :start_delayed_job_worker do
  comment 'Start new delayed_job worker'
  command %{RAILS_ENV=production /var/www/beerbutton/current/bin/delayed_job start}
end
